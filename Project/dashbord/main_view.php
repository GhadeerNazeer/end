			...
			<div class="col-12">
				<table class="table table-hover">
					...
					<tbody>
						...
						$query = "SELECT * from main limit $limit offset $offset";
						$result = mysqli_query($connection, $query);
       
						if (mysqli_num_rows($result) > 0) {
							while ($row = mysqli_fetch_assoc($result)) {
								echo '<tr>' . '<td>' . $row['id'] . '</td>' . '<td>' . $row['name'] . '</td>' . '<td>' . $row['job'] . '</td>'.'<td>' . $row['description'] . '</td>' .'<td>' . $row['facebook'] . '</td>'.'<td>' . $row['twitter'] .'<td>' . $row['linkedin'] . '</td>'.'<td>' . $row['github'] . '</td>'.'<td>' . $row['image'] . '</td>'.'</td>'. '<td><a href="main_edit.php?id=' . $row['id'] . '">Edit</a></td>' . '<td><form action="main_delete.php" method="POST"><input type="hidden" name="id" value="' . $row['id'] . '"> <button type="button" class="btn btn-danger" id="delete-btn"0>DELETE</button> </form></td>' . '</tr>';
							}
						}
